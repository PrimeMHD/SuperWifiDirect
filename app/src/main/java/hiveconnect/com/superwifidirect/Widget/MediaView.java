...
public abstract class MediaView extends View implements ValueAnimator.AnimatorUpdateListener {
    ...
    protected int initialShadowRadius ;
    ...
    public MediaView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        radius = array.getDimensionPixelSize(R.styleable.MediaView_radius, 20);
        shadowRadius = array.getDimensionPixelSize(R.styleable.MediaView_shadowRadius, 1);
        ...
        initialShadowRadius=array.getDimensionPixelSize(R.styleable.MediaView_initialShadowRadius, 1);
        array.recycle();
        ...
        if(initialShadowRadius<=0){
            initialShadowRadius=1;
        }
    ...
    private void updateAnim() {
        pressAnim.setIntValues(initialShadowRadius, shadowRadius + shadowRadius * 2 / 3, shadowRadius);
        releaseAnim.setIntValues(shadowRadius, initialShadowRadius);
    }
    ...
    protected final void onDraw(Canvas canvas) {
        ...
        if(shadowRadiusForAnim!=1)
            paint.setMaskFilter(new BlurMaskFilter(shadowRadiusForAnim, BlurMaskFilter.Blur.SOLID));
        else
            paint.setMaskFilter(new BlurMaskFilter(initialShadowRadius,BlurMaskFilter.Blur.SOLID));
